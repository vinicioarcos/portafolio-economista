...
- La portada no ganó dos secciones más: dado que ya es larga (problema señalado explícitamente en el brief), Libros y Blog se enlazan desde la sección de Docencia ("Ver libros") y desde el Footer, no como secciones propias. En la Fase 6 sí se agregaron al nav principal, una vez que quitar "Proyectos" dejó espacio.
...
## 3.7 Fase 7 aplicada (2026-08-27): SEO global

- `layout.tsx`: `metadataBase`, canonical de portada, plantilla de título (`"%s | Vinicio Arcos"`), Open Graph, Twitter Card y JSON-LD `Person` (nombre, cargo, `worksFor`/`alumniOf` derivados de `experience.ts`/`education.ts` ya verificados, `sameAs` limitado a ORCID, OSF, GitHub Pages y el LinkedIn que ya mostraba el sitio — no se agregó ningún perfil nuevo sin verificar).
- Se quitó el sufijo manual "| Vinicio Arcos" de cada `title` de subpágina (ahora lo agrega la plantilla del layout) para no duplicarlo, y se agregó `alternates.canonical` a cada ruta (`/perfil`, `/investigacion`, `/publicaciones`, `/docencia`, `/libros`, `/blog`, `/blog/[slug]`).
- Nuevos `robots.ts` y `sitemap.ts` (convenciones de App Router) con todas las rutas estáticas más los 3 posts del blog.
- Nueva `opengraph-image.tsx`: tarjeta social generada dinámicamente (colores de marca, sin foto) en vez de usar `public/images/profile.jpg`, que es un placeholder de apenas 141×130 px — insuficiente para una vista previa social. **Pendiente:** reemplazar por una fotografía profesional de alta resolución cuando esté disponible.
- **Verificación visual real ejecutada** (no solo HTML): se instaló Playwright localmente y se tomaron capturas de pantalla del sitio corriendo en `next start`, en escritorio (1440px) y móvil (390px), de la portada y las 6 subpáginas. Se revisaron visualmente — no solo se listaron como "200 OK" — y se confirmó: el nav de 8 ítems no se desborda ni se aprieta a 1440px; el menú hamburguesa móvil abre y lista los 8 ítems correctamente; `console --errors` no reportó ningún error en la portada. Este paso detectó y corrigió un bug real: un `next build` que quedó desactualizado tras editar el Navbar, que habría hecho que el commit se publicara sin "Libros" y "Blog" en el nav si no se hubiera revisado visualmente.
- `robots.txt` y `sitemap.xml` verificados por `curl` (contenido correcto, `Content-Type` correcto); imagen OG verificada devolviendo `image/png`.
